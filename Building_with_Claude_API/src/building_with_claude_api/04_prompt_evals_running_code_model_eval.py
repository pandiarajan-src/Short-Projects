...
The evaluation is only model based, and does not include human or code evaluation. 
...
def run_prompts(test_case):
    ...
    prompt = f"""
        ...
        Solution Criteria to Evaluate Against: {test_case["solution_criteria"]}
        
        * Respond only with {test_case['format'].lower()} format
        * Use the solution criteria provided to guide your response
...
def grade_syntax(test_case, output):
    """
    Grade the output by checking if it is valid Python, JSON, or Regex based on the expected format in the test case.
    """
    def validate_json(text):
        """
        Validate if the given text is a valid JSON string.
    ...
    def validate_python_code(text):
        """
        Validate if the given text is valid Python code.
        Returns True if valid, False otherwise.
        """
        try:
            ast.parse(text)
    ...
    def validate_regex(text):
        """
        Validate if the given text is a valid regular expression.
        Returns True if valid, False otherwise.
        """
        try:
            re.compile(text)
    ...
    if expected_format == "json":
        ...
        return validate_python_code(output)
...
def grade_by_model(test_case, output):
    ...
    grading_prompt = f"""
        ...
        Solution Criteria to Evaluate Against:
        <criteria>
    ...
    return parse_model_json(evaluation_output)  # Convert the JSON string to a Python dictionary
...
def run_test_cases(test_case):
    ...
    model_score = model_grade.get("score", 0)  # Default to 0 if score is not present
